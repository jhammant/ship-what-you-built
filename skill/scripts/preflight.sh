...
git rev-parse --is-inside-work-tree >/dev/null 2>&1 || {
  bad "Not a git repository. Run 'git init' first, or see guide/07-github.md."
  exit 2
}
...
head1 "4. .gitignore"
if [ ! -f .gitignore ]; then
  bad "There is no .gitignore. Write one before the first commit — see guide/07-github.md."
  found
